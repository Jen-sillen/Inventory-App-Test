...
const NewSaleForm: React.FC<NewSaleFormProps> = ({ onSuccess }) => {
  const { data, addSaleTransaction } = useData();
  const form = useForm<z.infer<typeof newSaleFormSchema>>({
    resolver: zodResolver(newSaleFormSchema),
    defaultValues: {
      dealerId: "",
      employeeId: "",
      productsSold: [{ sku: "", quantity: 1, price: 0 }],
    },
  });
  ...
  const onSubmit = (values: z.infer<typeof newSaleFormSchema>) => {
    ...
    try {
      addSaleTransaction(newSale);
      showSuccess("Sale recorded successfully!");
      form.reset();
      onSuccess?.();
    } catch (error: any) {
      showError(error.message || "Failed to record sale.");
    }
  };
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {data.products.filter(p => !p.isBulk).map((product) => ( // Only show non-bulk products for sale
